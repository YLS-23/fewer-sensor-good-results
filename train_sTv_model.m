function [trainedModel,validationRMSE,validationMSE] = train_sTv_model(trainingTbl,idx_selected_predictors)
% stepwiselm(...,'quadratic','Upper','quadratic','NSteps',1000,'Verbose',0)
% 10 fold cross validation, PRemove = 0.1

% Extract predictors and response
% This code processes the data into the right shape for training the model.
inputTable = trainingTbl;
predictorNames = {'180', '187.5', '195', '202.5', '210', '217.5', '225', '232.5', '240', '247.5', '255', '262.5', '270'};
predictorNames = predictorNames(idx_selected_predictors); % 选用其中部分
predictors = inputTable(:, predictorNames);
response = inputTable.T_value;

% Train a regression model
% This code specifies all the model options and trains the model.
concatenatedPredictorsAndResponse = predictors;
concatenatedPredictorsAndResponse.T_value = response;
...
for fold = 1:KFolds
    ...
    concatenatedPredictorsAndResponse.T_value = trainingResponse;
    linearModel = stepwiselm(...
    concatenatedPredictorsAndResponse, ...
    'quadratic', ...
    'Upper', 'quadratic', ...
